...
public final class GuiEngine {
    ...
    public enum Alignment {
        TOP_LEFT((pP, pS, m, s) -> new Vector2f(pP.x + m.x,
                pP.y + m.y)),
        TOP((pP, pS, m, s) -> new Vector2f((pS.x - pP.x - pP.z) / 2 - (s.x + m.x + m.z) / 2 + m.x,
                pP.y + m.y)),
        TOP_RIGHT((pP, pS, m, s) -> new Vector2f(pS.x - m.z - s.x - pP.z,
                pP.y + m.y)),
        LEFT((pP, pS, m, s) -> new Vector2f(pP.x + m.x,
                (pS.y - pP.y - pP.w) / 2 - (s.y + m.y + m.w) / 2 + m.y)),
        CENTER((pP, pS, m, s) -> new Vector2f((pS.x - pP.x - pP.z) / 2 - (s.x + m.x + m.z) / 2 + m.x,
                (pS.y - pP.y - pP.w) / 2 - (s.y + m.y + m.w) / 2 + m.y)),
        RIGHT((pP, pS, m, s) -> new Vector2f(pS.x - m.z - s.x - pP.z,
                (pS.y - pP.y - pP.w) / 2 - (s.y + m.y + m.w) / 2 + m.y)),
        BOTTOM_LEFT((pP, pS, m, s) -> new Vector2f(pP.x + m.x,
                pS.y - m.w - s.y - pP.w)),
        BOTTOM((pP, pS, m, s) -> new Vector2f((pS.x - pP.x - pP.z) / 2 - (s.x + m.x + m.z) / 2 + m.x,
                pS.y - m.w - s.y - pP.w)),
